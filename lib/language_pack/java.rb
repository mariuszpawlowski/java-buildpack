...
module LanguagePack
  class Java
    ...
    DEFAULT_JDK_VERSION = "1.17".freeze
    ...
    JDK_URL_1_8="https://intiuss-suite-jdk.s3-eu-west-1.amazonaws.com/jdk8.tar.gz".freeze
    ...
    JDK_URL_1_17="https://intiuss-suite-jdk.s3-eu-west-1.amazonaws.com/jdk17.tar.gz".freeze
    ...
    def install_java
      ...
      unless File.exists?("#{jdk_dir}/bin/java")
		...
		if File.directory?("#{jdk_overlay_dir}")
			puts "JDK Overlay exists, will copy files"
			copy_jdk_overlay
		end
    end
	
	def copy_jdk_overlay
      # Copy JDK overlay
      run_with_err_output("cp -rf #{jdk_overlay_dir}/* #{jdk_dir}")
    end
